...
arranging_rules(){
    ...
    find ${DOCU_PATH}/downloaded -mindepth 1 -maxdepth 1 -not -name "dev" -exec rm -r {} \;

#    mv ${DOCU_PATH}/downloaded/dev/* ${DOCU_PATH}/downloaded/


    find ${DOCU_PATH}/downloaded/dev/ -mindepth 1 -maxdepth 1 -type f \
        -not \( -name "about.html" -not -name "bugs.html" -not -name "contents.html" -not -name "copyright.html" -not -name "glossary.html" -not -name "license.html" \) -exec rm -r {} \;
...
parsing_rules(){
    ...
    #
    write_html_docu_multirule -f "h1" -b "section" -cp -lp -c "105"
    ...
    sed -i '12a\'$'\n''&@ g:dan_kw_question_list = "^See Also" @&' "${MAIN_TOUPDATE}"
    sed -i '13a\'$'\n''&@ g:dan_kw_nontext_list = "^Note" @&' "${MAIN_TOUPDATE}"
    sed -i '14a\'$'\n''&@ g:dan_kw_linenr_list = "" @&' "${MAIN_TOUPDATE}"
    sed -i '15a\'$'\n''&@ g:dan_kw_warningmsg_list = "^Changed in" @&' "${MAIN_TOUPDATE}"
