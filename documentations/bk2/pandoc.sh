...
arranging_rules(){
    ...
    rm ${DOCU_PATH}/downloaded/MANUAL.html

##    ## Brinding down the manual
##    mkdir -p ${DOCU_PATH}/downloaded/manual
##    mv ${DOCU_PATH}/downloaded/demo/example33/* ${DOCU_PATH}/downloaded/manual/

    rm -r ${DOCU_PATH}/downloaded/demo
...
parsing_rules(){
    ...
    #
    write_html_docu_multirule -f "h1.title" -f "h1" -f "h2" -b "body" -b "main" -cp -lp -c "105"
    ...
    sed -i '12a\'$'\n''&@ g:dan_kw_question_list = "^Since:,^Usage:" @&' "${MAIN_TOUPDATE}"
    ...
    sed -i '23a\'$'\n''&@ g:dan_kw_cursorline_list = "^Fields:,^ Module .*$" @&' "${MAIN_TOUPDATE}"
