...
arranging_rules(){
    ...
    find ${DOCU_PATH}/downloaded/ -mindepth 1 -maxdepth 1 -not -name "linux" -exec rm {} -r \;
    find ${DOCU_PATH}/downloaded/linux/man-pages -mindepth 1 -maxdepth 1 -type f -exec rm {} \;


#    find ${DOCU_PATH}/downloaded/linux/man-pages/ -mindepth 1 -maxdepth 1 -type d -exec mv {} ${DOCU_PATH}/downloaded/ \;
...
file_mangling=$(cat <<'EOF'
tail -n +14 | head -n -13 | sed -e '1i ```man' -e '$a ```'
EOF
)

    write_html_docu_multirule -f "h1" -b "body" -fm "$file_mangling" -lp -c "105"
...
#    sed \
#        -e '/^  ------------------------------ ----------------------------------------$/d' \
#        -e '/^  man7.org > Linux > man-pages   Linux/UNIX system programming training$/d' \
#        -e '/^------------------------------------------------------------------------$/d' \
#        -e '/^+-----------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------------------+$/d' \
#        -e '/^+-----------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------------------+$/d' \
#        -e '/^|                                                                                                                                                                 |                                   |$/d' \
#        -e '/^+-----------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------------------+$/d' \
#        -e '/^|                                                                                                    |                       |                       |$/d' \
#        -e '/^| For details of in-depth Linux/UNIX system programming training courses that I teach, look here .   |                       |                       |$/d' \
#        -e '/^| Hosting by jambit GmbH .                                                                           |                       |                       |$/d' \
#        -i "${MAIN_TOUPDATE}"
