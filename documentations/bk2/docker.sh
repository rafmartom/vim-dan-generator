...
arranging_rules(){
    ...
    find ${DOCU_PATH}/downloaded -mindepth 1 -maxdepth 1 -type f -exec rm {} \;
    rm -r ${DOCU_PATH}/downloaded/tags
...
parsing_rules(){
    ...
    #
    write_html_docu_multirule -f "article h1" -b "article" -cp -lp -c "105"
    ...
    sed -i '12a\'$'\n''&@ g:dan_kw_question_list = "^Examples" @&' "${MAIN_TOUPDATE}"
    sed -i '13a\'$'\n''&@ g:dan_kw_nontext_list = "" @&' "${MAIN_TOUPDATE}"
    sed -i '14a\'$'\n''&@ g:dan_kw_linenr_list = "" @&' "${MAIN_TOUPDATE}"
    sed -i '15a\'$'\n''&@ g:dan_kw_warningmsg_list = "^Prerequisites" @&' "${MAIN_TOUPDATE}"
    sed -i '16a\'$'\n''&@ g:dan_kw_colorcolumn_list = "" @&' "${MAIN_TOUPDATE}"
    sed -i '17a\'$'\n''&@ g:dan_kw_underlined_list = "^Upgrades" @&' "${MAIN_TOUPDATE}"
    sed -i '18a\'$'\n''&@ g:dan_kw_preproc_list = "^New" @&' "${MAIN_TOUPDATE}"
    sed -i '19a\'$'\n''&@ g:dan_kw_comment_list = "^Options" @&' "${MAIN_TOUPDATE}"
    ...
    sed -i '24a\'$'\n''&@ g:dan_kw_tabline_list = "^Description" @&' "${MAIN_TOUPDATE}"
