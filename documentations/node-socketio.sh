#!/bin/bash 
# @file node-socketio
# @brief vim-dan ruleset file for documentation on Socket.io library
# @description
...
filtering_rules() {
    ...
    for DOWNLOAD_LINK in "${DOWNLOAD_LINKS[@]}"; do
        ...
        sed -i '\|socket[.]io/pt-br|d' ${LOCAL_CSV_PATH}
        sed -i '\|socket[.]io/zh-CN|d' ${LOCAL_CSV_PATH}
    done
...
EOF
    ...
    write_html_docu_multirule -f "h1" -b "article" -cd "javascript" -il -c "105" -cc "${cleanup_command}"
