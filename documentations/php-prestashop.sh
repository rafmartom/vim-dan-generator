#!/bin/bash 
# @file php-prestashop
# @brief vim-dan ruleset file for documentation on php-prestashop
# @description
...
filtering_rules() {
    ...
    for DOWNLOAD_LINK in "${DOWNLOAD_LINKS[@]}"; do
        ...
        incl_1=$(mktemp); sed -n '\|devdocs[.]prestashop-project[.]org/9/|p' "${LOCAL_CSV_PATH}" > "$incl_1"
        ...
        sed -i '\|%60%20+%20element[.]url%20+%20%20%60|d' ${LOCAL_CSV_PATH}
    done
...
parsing_rules(){
    ...
    parse_html_docu_multirule -f "h1" -b "#toc, #body-inner"
...
EOF
    ...
    write_html_docu_multirule  -f "h1" -b "#toc, #body-inner" -cd "php" -il -c "105" -cc "${cleanup_command}"
