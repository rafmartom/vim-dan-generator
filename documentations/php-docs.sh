#!/bin/bash 
# @file php-docs
# @brief vim-dan ruleset file for documentation on php-docs
# @description
...
#    echo 'unzip jdk-8u451-docs-all.zip -d $HOME/downloads/vim-dan/oracle-java/downloaded/'
    ...
    wget -P "${DOCU_PATH}" https://www.php.net/distributions/manual/php_manual_en.tar.gz

    mkdir -p ${DOCU_PATH}/downloaded

    tar -xzf "${DOCU_PATH}/php_manual_en.tar.gz" -C "${DOCU_PATH}/downloaded"
...
parsing_rules(){
    ...
    parse_html_docu_multirule -f "failedselector" -b "#breadcrumbs, #layout" 
...
#        -i "${MAIN_TOUPDATE}"
    ...
    #    write_html_docu_multirule -f "h1" -b "article" -cd "php"
    #
    #  Other Example:
    ...
    #
    #    write_html_docu_multirule -f "head title" -b "div.guide-content" -b "body" -cd "php"
    #
    

    write_html_docu_multirule  -f "failedselector" -b "#breadcrumbs, #layout" -cd "php" -il -c "105"
