#!/bin/bash 
# @file govuk-rest
# @brief vim-dan ruleset file for documentation on govuk-rest
# @description
...
arranging_rules(){
    ...
    find ${DOCU_PATH}/../govuk-parent/downloaded/www.gov.uk -mindepth 1 -maxdepth 1 -type d \
        -not \( -name "government" \) \
        -exec cp -r {} ${DOCU_PATH}/downloaded/ \;

    find ${DOCU_PATH}/../govuk-parent/downloaded/www.gov.uk/government -mindepth 1 -maxdepth 1 -type d \
        -not \( -name "publications" \) \
        -exec cp -r {} ${DOCU_PATH}/downloaded/government \;
...
parsing_rules(){
    ...
    parse_html_docu_multirule -f "h1.govuk-heading-l" -b "main"
...
EOF
    ...
    write_html_docu_multirule -f "h1.govuk-heading-l" -b "main" -cd "sh" -il -cc "${cleanup_command}"
