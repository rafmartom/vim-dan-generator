#!/bin/bash 
# @file sqlzoo
# @brief vim-dan ruleset file for documentation on SQL Zoo and non-SQL zoo
# @description
...
filtering_rules() {
    ...
    for DOWNLOAD_LINK in "${DOWNLOAD_LINKS[@]}"; do
        ...
        if [[ ${DOWNLOAD_LINK} == "https://sqlzoo.net/"  ]]; then
            ## WRITE BELOW YOUR INCLUSION RULES
            sed -ni '\|sqlzoo[.]net|p' ${LOCAL_CSV_PATH}

            ## WRITE BELOW YOUR EXCLUSION RULES
            #sed -i '\|developer[.]mozilla[.]org/es|d' ${LOCAL_CSV_PATH}
        fi

        if [[ ${DOWNLOAD_LINK} == "https://nosqlzoo.net/"  ]]; then
            ## WRITE BELOW YOUR INCLUSION RULES
            sed -ni '\|nosqlzoo[.]net|p' ${LOCAL_CSV_PATH}
...
arranging_rules(){
    ...
    find ${DOCU_PATH}/downloaded/sqlzoo.net/ -mindepth 1 -maxdepth 1 -not -name "wiki" -not -name "index.html" -exec rm -r {} \;

    find ${DOCU_PATH}/downloaded/sqlzoo.net/ -regex '.*/\(de\|ja\|zh\|fr\|es\)\.html' -exec rm {} \;
    

    find ${DOCU_PATH}/downloaded/nosqlzoo.net/ -mindepth 1 -maxdepth 1 -not -name "wiki" -not -name "index.html" -exec rm -r {} \;
...
parsing_rules(){
    ...
    parse_html_docu_multirule -f "h1" -b "#bodyContent"
...
#        -i "${MAIN_TOUPDATE}"
    ...
    #    write_html_docu_multirule -f "h1" -b "article" -cd "sql"
    #
    #  Other Example:
    ...
    #
    #    write_html_docu_multirule -f "head title" -b "div.guide-content" -b "body" -cd "sql"
    #
    

    write_html_docu_multirule -f "h1" -b "#bodyContent" -cd "sql" -il -c "105"
