...
filtering_rules() {
    ...
    for DOWNLOAD_LINK in "${DOWNLOAD_LINKS[@]}"; do
        ...
        sed -ni '\|beautiful-soup-4[.]readthedocs[.]io/en/latest/index[.]html|p' ${LOCAL_CSV_PATH}
...
parsing_rules(){
    ...
    parse_html_docu_multirule -f ".wy-breadcrumbs li" -b ".wy-side-scroll, .wy-nav-content"
...
EOF
    ...
    write_html_docu_multirule -f ".wy-breadcrumbs li" -b ".wy-side-scroll, .wy-nav-content" -cd "python" -il -c "105" -cc "${cleanup_command}"
