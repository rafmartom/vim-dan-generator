#!/bin/bash 
# @file ruby-lang
# @brief vim-dan ruleset file for documentation on ruby-lang
# @description
...
filtering_rules() {
    ...
    for DOWNLOAD_LINK in "${DOWNLOAD_LINKS[@]}"; do
        ...
        incl_1=$(mktemp); sed -n '\|docs[.]ruby-lang[.]org/en/3[.]4|p' ${LOCAL_CSV_PATH} > "$incl_1"
        incl_2=$(mktemp); sed -n '\|docs[.]ruby-lang[.]org/en/master|p' ${LOCAL_CSV_PATH} > "$incl_2"
...
EOF
    ...
    write_html_docu_multirule -f "h1" -b "main" -cd "ruby" -il -c "105" -cc "${cleanup_command}"
